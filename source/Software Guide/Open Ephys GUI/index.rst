...
---------------------------------------------
There is currently no formal ONIX plugin available for the `Open Ephys GUI
...
This is fine when the majority of the data being processed is synchronized
ephys and auxiliary data. However, by design, ONIX hardware makes no such
guarantees about the nature of the data it produces. On the contrary, an
...
these devices is individually time-stamped in hardware, there is guarantee when
they will arrive and in what order. This necessitates the use of event-driven
acquisition software that only propagates data when its received, and this
is where `Bonsai <https://bonsai-rx.org/>`__ really shines. For this reason, we
have dedicated the majority of our development effort toward the
...
see if they have their probe in the right spot. We hope to changes this
